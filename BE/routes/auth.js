...
router.post("/login", async function (req, res, next) {
  let username = req.body.username; //get từ FE xuống
  ...
  if (checkpass) {
    ...
    ResHelper.ResponseSend(res, true, 200, { user: userInfo });
  } else {
    ResHelper.ResponseSend(
      res,
      false,
...
router.post("admin/login", async function (req, res, next) {
  ...
  if (!user.role.includes("admin")) {
    ResHelper.ResponseSend(res, false, 404, "Bạn không có quyền");
    return;
  }
